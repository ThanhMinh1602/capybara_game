  ...
  import 'package:capybara_game/features/player/presentations/widgets/slider_widget.dart';
  ...
  class _PlayerWidgetState extends State<PlayerWidget> {
      ...
      return BlocBuilder<PlayerBloc, PlayerState>(builder: (context, state) {
        ...
        return Column(
          children: [
            ...
            _buildGridCard(level, config, datas, state, config.maxScore)
          ],
        );
      });
    }
    ...
    Widget _buildGridCard(int level, GridConfigModel config,
        List<CardModel> datas, PlayerState state, double maxScore) {
      return Expanded(
        child: GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return _buildCardItem(
                context, card, state, level, datas, index, maxScore);
          },
        ),
      );
    }
    ...
    Widget _buildCardItem(BuildContext context, CardModel card, PlayerState state,
        int level, List<CardModel> datas, int index, double maxScore) {
      return GestureDetector(
        onTap: () {
          context
              .read<PlayerBloc>()
              .add(PlayerEvent.tapCard(card, state.tries, level, maxScore));
        },
        child: Container(
          decoration: BoxDecoration(
            boxShadow: [
    ...
    Widget _buildScoreItem({int? point, required String iconPath}) {
      return Container(
        width: 120.w,
        height: 60.h,
